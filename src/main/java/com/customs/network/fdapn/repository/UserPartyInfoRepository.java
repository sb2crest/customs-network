...
public interface UserPartyInfoRepository extends JpaRepository<UserPartyInfo, Long> {
    @Query("SELECT u FROM UserPartyInfo u WHERE u.partyIdentifierId = :partyIdentifierId AND u.uniqueUserIdentifier = :uniqueUserIdentifier")
    Optional<UserPartyInfo> findByUniqueUserIdentifierAndPartyIdentifierId(@Param("uniqueUserIdentifier") String uniqueUserIdentifier, @Param("partyIdentifierId") String partyIdentifierId);
    List<UserPartyInfo> findByUniqueUserIdentifier(String uniqueUserIdentifier);
